...
while [[ "$CURRENT_DATE" != $(date -I -d "$END_DATE + 1 day") ]]; do
    ...
    if [[ -z "$MIN_ID" || -z "$MAX_ID" ]]; then
        ...
        # 使用 mysqldump 根據 ID 和 created_at 條件進行匯出，並用 pigz 做高效壓縮
        echo "匯出並壓縮 $CURRENT_DATE 的數據 (ID: $MIN_ID-$MAX_ID) 到 $OUTPUT_FILE"
        mysqldump -u $DB_USER -p$DB_PASSWORD $DB_NAME $TABLE_NAME \
            --where="id >= $MIN_ID AND id <= $MAX_ID AND created_at >= '$START_TIME' AND created_at <= '$END_TIME'" \
            --single-transaction --quick --skip-lock-tables --no-create-info \
            --skip-add-locks --skip-comments | pigz -9 -p 4 -b 128 > $OUTPUT_FILE
    fi
